    ...
    */
    ...
    static bool HandleWpModifyCommand(ChatHandler* handler, char const* args)
    {
        ...
        if (text == nullptr)
        {
            ...
            WorldDatabase.PExecute("UPDATE waypoint_data SET %s=NULL WHERE id='{}' AND point='{}'", show_str, pathid, point); // Query can't be a prepared statement
        }
        else
        {
            // show_str check for present in list of correct values, no sql injection possible
            ...
            WorldDatabase.PExecute("UPDATE waypoint_data SET %s='%s' WHERE id='{}' AND point='{}'", show_str, text2.c_str(), pathid, point); // Query can't be a prepared statement
        }
